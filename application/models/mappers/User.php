...
class Application_Model_Mapper_User{
	...
	public function getAllClientActive(){
		...
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()
														->where(" UserRole = ?", Application_Model_DbTable_Users::USER_CLIENT)
														->where(" UserIsActive = ?", 'Y')
														->orWhere(" UserLoginName = ?", $config->fmsadmin->username)
														->order("UserID desc"));
	...
	public function getAvailableJtepmAccount(){
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()
														->where(" UserRole  = ?", Application_Model_DbTable_Users::USER_JTEPM)
														->where(" UserIsActive= ?", 'Y')
														->order("UserID desc"));
	...
	public  function getAvailableJtepmEmail(){				
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()->from("users", "UserEmail")
													->where(" UserRole  = ?", Application_Model_DbTable_Users::USER_JTEPM)
													->where("UserIsActive=?", 'Y')
													->order("UserID desc"));
