...
class Application_Model_Mapper_Language{
	...
	public function getSourceLanguageActive(){
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()
													->where(" LanguageIsActive = ?", 'Y')
													->where(" LanguageIsShowInSourceList= ?", 'Y')
													->order("LanguageName asc"));
	...
	public function getTargetLanguageActive(){
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()
														->where(" LanguageIsActive = ?", 'Y') 
														->where(" LanguageIsShowInTargetList= ?",'Y')
														->order("LanguageName asc"));
    
		return $this->CreateResultSetEntity($resultSet);
	}
	
	public function getLanguageIsShowInTranslatorListActive(){
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()->where("(LanguageIsActive = 'Y') and (LanguageIsShowInTranslatorList='Y')")->order("LanguageName asc"));
	...
	public function save(Application_Model_Language $language){			
		$data = array(
			...
			'LanguageIsShowInTranslatorList' => $language->getLanguageIsShowInTranslatorList()
        );
