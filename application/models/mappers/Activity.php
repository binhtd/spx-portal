...
class Application_Model_Mapper_Activity{
	...
	private function createWhereCondition($userid, $startDate, $endDate){
		...
		if (($userid != null) && ($userid != "all")){
			$where .= $this->getDbTable()->getAdapter()->quoteInto(" and UserName = ?", $userid);
		}
		 
		if($startDate != null){
			$where .= $this->getDbTable()->getAdapter()->quoteInto(" and UserActivityDateTime >= ?", $startDate);
		}
		
		if($endDate != null){
			$where .= $this->getDbTable()->getAdapter()->quoteInto(" and UserActivityDateTime <= ?", $endDate);
		}
	...
	public function getDailyReport(){
		...
		$resultSet = $this->getDbTable()->fetchAll($this->getDbTable()->select()
							->where(" UserActivityDateTime >= ?", $startDate)
							->where(" UserActivityDateTime <= ?", $endDate)
							->where(" Module = ?", $module)
							->order("UserActivityDateTime desc")
							->order("UserName desc"));
